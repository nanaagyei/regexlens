...
<Callout type="info">
**Signed in:** The Analysis tab and `POST /api/analyze` require authentication on deployments that enable this route.
</Callout>
...
- **Analysis** — Aggregated **risk score** (0–100), **complexity** level (`low` → `extreme`), extra structured warnings, short **notes**, and **safe rewrite suggestions** where applicable.
...
- Sign-in prompt when the Analysis tab needs an authenticated session
